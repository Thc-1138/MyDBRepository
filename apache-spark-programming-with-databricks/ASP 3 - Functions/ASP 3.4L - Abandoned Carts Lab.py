...
conversions_df = (users_df.join(converted_users_df, "email", "outer").filter("email is not null").na.fill({"converted":"False"})
                 )
...
# TODO
carts_df = (events_df.withColumn("items", explode("items")).groupBy("user_id")
...
# TODO
abandoned_carts_df = (email_carts_df.where("converted = False and cart is not null")
)
